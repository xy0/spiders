        //  Requires JQuery and a canvas called "spiders"
       //
      //
     //  eg: <canvas height="605" width="1920" id="spiders" class="hidden-xs"></canvas>
    //
   //
  //           \_|\ ~C
 //
...
$( function() {
    ...
    function shiftPoint( p ) {
        TweenLite.to( p, 1 + 1 * Math.random(), {
            x: p.originX - 50 + Math.random() * 25,
            y: p.originY - 50 + Math.random() * 25,
            onComplete: function() {
                shiftPoint( p );
            }
        } );
    ...
    function drawLines( p ) {
        ...
        for ( var i in p.closest ) {
            ...
            ctx.strokeStyle = 'rgba(120,120,200,' + p.active + ')';
            ctx.stroke();
        }
    }
